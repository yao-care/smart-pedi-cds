...
- **掃描器設定**：[`.zap/rules.tsv`](../../.zap/rules.tsv)（單一真相源，與本文件對齊）
...
上述 7 項在現行 GitHub Pages 託管架構下無法以應用程式層修補，於本系統威脅模型（靜態、零後端、無 PII、無登入）下殘餘風險評為**低**，由營運主體（藥提醒科技有限公司）**接受**並以本文件 + `.zap/rules.tsv` 留存。
